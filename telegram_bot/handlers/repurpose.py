...
async def handle(update, context):
    ...
    if not txt and update.message.reply_to_message:
        txt = update.message.reply_to_message.text or ""

    if not txt:
        return await update.message.reply_text("Daj tekst.")

    chat_svc, auth_svc = context.bot_data["chat_service"], context.bot_data["auth_service"]
    res = await with_auth_retry(
        lambda uid, tok: chat_svc.send_message(uid, tok, f"Repurpose this: {txt}"),
        update,
        context,
        auth_svc,
    )
