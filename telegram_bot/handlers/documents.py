...
async def handle(update, context):
    ...
    with tempfile.NamedTemporaryFile(suffix=os.path.splitext(doc.file_name)[1], delete=False) as tmp:
        tmp_path = tmp.name
    try:
        f = await context.bot.get_file(doc.file_id)
        await f.download_to_drive(tmp_path)
        await with_auth_retry(
            lambda uid, tok: rag_svc.upload_file(tok, tmp_path, doc.file_name),
            update,
            context,
            auth_svc,
        )
