...
class AdminService:
    async def get_overview(self, db: AsyncSession) -> dict[str, Any]:
        ...
        active_today = await db.scalar(
            select(func.count(User.id)).where(func.date(User.last_seen_at) == today)
        )
        total_cost_today = await db.scalar(
            select(func.coalesce(func.sum(UsageLedger.cost_usd), 0)).where(
                func.date(UsageLedger.created_at) == today
            )
        )
    ...
    async def change_role(
        self, telegram_id: int, new_role: str, db: AsyncSession
    ) -> dict[str, Any]:
