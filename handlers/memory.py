...
async def handle_new_conversation(update, context): await handle_forget(update, context)
async def handle_export(update, context):
    ...
    if not cid: return await update.message.reply_text("Brak aktywnej rozmowy.")
    res = await with_auth_retry(lambda uid, tok: chat_svc.export_conversation(tok, cid), update, context, auth_svc)
