...
from utils.message_splitter import should_send_as_file, split_message
    ...
    async with TypingIndicator(update, context):
        try:
            res = await with_auth_retry(
                lambda uid, tok: chat_svc.send_message(
                    uid,
                    tok,
                    update.message.text,
            ...
            footer = format_meta_footer(
                res.get("model_name", "AI"),
                res.get("cost_usd", 0),
                total_tokens,
                res.get("_elapsed", 0),
                res.get("fallback", False),
            )
            full = res.get("response", "") + footer
            if should_send_as_file(full):
                from io import BytesIO

                bio = BytesIO(full.encode())
