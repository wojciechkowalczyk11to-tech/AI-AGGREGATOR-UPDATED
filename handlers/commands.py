...
async def handle_model_command(update, context):
    ...
    if not context.args:
        ...
        await with_auth_retry(lambda uid, tok: auth_svc.update_settings(uid, tok, {"preferred_model": prov}), update, context, auth_svc)
    ...
    async with TypingIndicator(update, context):
        res = await with_auth_retry(lambda uid, tok: chat_svc.send_with_provider(uid, tok, " ".join(context.args), prov, conversation_id=context.user_data.get("conv_id")), update, context, auth_svc)
        context.user_data["conv_id"] = res.get("conversation_id")
        footer = format_meta_footer(res.get("model_name", prov), res.get("cost_usd", 0), (res.get("input_tokens",0)+res.get("output_tokens",0)), res.get("_elapsed", 0))
