...
async def ensure_authenticated(update, context, auth_service, force_refresh=False):
    ...
    if user_id and token and not force_refresh: return user_id, token
    if not update.effective_user: raise BackendServiceError("No user")
    result = await auth_service.register(update.effective_user.id)
    user_id, token = str(result["id"]), result["access_token"]
    context.user_data.update({"backend_user_id": user_id, "access_token": token, "tier": result.get("subscription_tier", "basic")})
...
async def with_auth_retry(coro_factory, update, context, auth_service):
    ...
    try: return await coro_factory(user_id, token)
